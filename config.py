...
def _s(key, default=""):
    ...
    2. st.secrets[key]         Streamlit Cloud (يُستدعى فقط إذا كان Streamlit شغّالاً)
    ...
    if v:
        return v
    # 2. st.secrets — فقط إذا كان Streamlit runtime نشطاً (ليس أثناء Build)
    try:
        # نتحقق من وجود runtime قبل أي استدعاء لـ streamlit
        import sys
        if "streamlit" in sys.modules:
            import streamlit as _st
            # نتأكد أن الـ runtime شغّال فعلاً (وليس مجرد import)
            if hasattr(_st, "runtime") and hasattr(_st.runtime, "exists"):
                if _st.runtime.exists():
                    v = _st.secrets.get(key, None)
                    if v is not None:
                        return str(v) if not isinstance(v, (list, dict)) else v
            else:
                # Streamlit < 1.12 fallback
                v = _st.secrets.get(key, None)
                if v is not None:
                    return str(v) if not isinstance(v, (list, dict)) else v
    except Exception:
